...
const (
	EmergencyDesc     = "EMER" // 系统级紧急，比如磁盘出错，内存异常，网络不可用等
	AlertDesc         = "ALRT" // 系统级警告，比如数据库访问异常，配置文件出错等
	CriticalDesc      = "CRIT" // 系统级危险，比如权限出错，访问异常等
	ErrorDesc         = "EROR" // 用户级错误
	WarningDesc       = "WARN" // 用户级警告
	InformationalDesc = "INFO" // 用户级信息
	DebugDesc         = "DEBG" // 用户级调试
	TraceDesc         = "TRAC" // 用户级基本输出
)
